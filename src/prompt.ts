...
export function getExtensionRoot(): string {
  ...
  if (typeof process !== "undefined" && process.pkg) {
    // 在 pkg 打包环境中，使用 process.cwd() 或可执行文件所在目录
    return path.dirname(process.execPath);
  }

  // 2. Prefer `__dirname` which is always available in the CJS bundle output.
  // 3. Fall back to `import.meta.url` for ESM test environments (tsx --test).
