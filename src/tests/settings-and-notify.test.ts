...
test("resolveSettingsSources merges MCP env with documented priority", () => {
  ...
  // 使用类型断言来通过 TypeScript 检查
  const githubServer = resolved.mcpServers?.github as {
    command?: string;
    args?: string[];
    env?: Record<string, string>;
  };
  assert.equal(githubServer?.command, "python");
  assert.deepEqual(githubServer?.args, ["user-server.js"]);
  assert.deepEqual(githubServer?.env, {
