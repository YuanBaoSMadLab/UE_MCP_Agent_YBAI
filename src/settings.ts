...
// 类型守卫函数
export function isHttpMcpServerConfig(config: McpServerConfig): config is { type: "http"; url: string } {
  return "type" in config && config.type === "http";
}

export function isStdioMcpServerConfig(
  config: McpServerConfig
): config is { type?: "stdio"; command: string; args?: string[]; env?: Record<string, string> } {
  return !isHttpMcpServerConfig(config);
}
...
function mergeMcpServers(
  ...
  // 如果用户没有配置任何 MCP 服务器，默认添加 UE5 MCP
  const hasNoServers = serverNames.size === 0;
  const shouldAddDefaultUe5 = hasNoServers || (!("ue5" in userServers) && !("ue5" in projectServers));
  ...
  for (const name of serverNames) {
    ...
    const effectiveConfig = projectConfig ?? userConfig;
    if (!effectiveConfig) {
      continue;
    }

    if ("url" in effectiveConfig) {
      merged[name] = { type: "http", url: effectiveConfig.url };
    } else {
      const command = effectiveConfig.command;

      // 处理用户配置的环境变量
      let userConfigEnv: Record<string, string> = {};
      if (userConfig && !("url" in userConfig) && userConfig.env) {
        userConfigEnv = userConfig.env;
      }

      // 处理项目配置的环境变量
      let projectConfigEnv: Record<string, string> = {};
      if (projectConfig && !("url" in projectConfig) && projectConfig.env) {
        projectConfigEnv = projectConfig.env;
      }
  ...
  // 如果应该添加默认的 UE5 配置且用户没有配置 ue5 服务器，则添加默认配置
  if (shouldAddDefaultUe5 && !("ue5" in merged)) {
    merged.ue5 = {
      type: "http",
