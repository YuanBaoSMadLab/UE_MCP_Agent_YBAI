...
### HTTP 传输模式（适用于 UE5.8 内置 MCP 插件等）
...
| 字段      | 类型     | 必填 | 说明                                                                                                                   |
| --------- | -------- | ---- | ---------------------------------------------------------------------------------------------------------------------- |
| `type`    | string   | 否   | 传输类型，可选值为 `stdio`（默认）或 `http`                                                                             |
...
| 字段    | 类型     | 必填 | 说明                                                                                                                   |
| ------- | -------- | ---- | ---------------------------------------------------------------------------------------------------------------------- |
| `type`  | string   | 是   | 传输类型，必须为 `http`                                                                                                 |
| `url`   | string   | 是   | MCP 服务器的 HTTP 端点 URL（如 `http://localhost:8000/mcp`）                                                           |
...
### Unreal Engine 5.8 内置 MCP 插件

让 Deep Code 直接操控 Unreal Engine 5.8 编辑器：
...
**设置说明：**
1. 在 Unreal Engine 5.8 中启用 `ModelContextProtocol` 和 `ToolsetRegistry` 插件
2. 配置插件的自动启动功能，设置端口为 8000
3. 启动 Unreal Engine 编辑器后即可使用

**可用功能：**
- 列出可用的工具集（toolsets）
- 描述工具集的详细信息
- 加载工具集
- 使用场景工具（SceneTools）添加几何体到场景中
- 查找场景中的 Actor
- 以及更多 Unreal 编辑器功能
