...
> **自动发现 UE5 MCP 工具集，减少 token 消耗，加速 AI 操作！**

YB-AI 内置了 **UE5.8 MCP Skill**，将 UE5.8 MCP 插件的能力封装为专业技能，AI 加载后能直接理解 UE5 MCP 的完整 API。
...
帮我列出工具集     # AI 自动调用 list_toolsets
```

### ✨ 效果

- **减少 Token 消耗**：无需每次都重新发现工具
- **加速工具调用**：AI 直接理解 MCP 调用格式
- **精准参数**：工具参数和 schema 已蒸馏优化

### 安装到其他 IDE

UE5.8 MCP Skill 也支持安装到其他 AI 编程助手！

一键安装（双击）：

```
packages/ue5-mcp-skill/install.bat
```

选择目标 IDE 即可自动安装。支持的 IDE：
- **DeepCode CLI** - `/ue5-mcp` 命令加载
- **Trae CN** - MCP 配置 + Skill 加载
- **CodeBuddy** - Skill 目录安装
- **CodeX (Codeium)** - Skill 目录安装

手动配置请查看：`packages/ue5-mcp-skill/README.md`
